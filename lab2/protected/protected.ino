...
std::vector<uint32_t> currentStepSize;
...
std::vector<uint32_t> decodeKeyPress_firstset(uint16_t key_array){
  std::vector<uint32_t> stepSizeReturn;
  switch(key_array){
    case 0xE:
      stepSizeReturn.push_back(stepSizes[0]);
      noteMessage[2] = intToHex[0];
      break;
    case 0xD:
      stepSizeReturn.push_back(stepSizes[1]);
      noteMessage[2] = intToHex[1];
      break;
    case 0xB:
      stepSizeReturn.push_back(stepSizes[2]);
      noteMessage[2] = intToHex[2];
      break;
    case 0x7:
      stepSizeReturn.push_back(stepSizes[3]);
...
std::vector<uint32_t> decodeKeyPress_secondset(uint16_t key_array){
  std::vector<uint32_t> stepSizeReturn;
  switch(key_array){
    case 0xE:
      stepSizeReturn.push_back(stepSizes[4]);
      noteMessage[2] = intToHex[4];
      break;
    case 0xD:
      stepSizeReturn.push_back(stepSizes[5]);
      noteMessage[2] = intToHex[5];
      break;
    case 0xB:
      stepSizeReturn.push_back(stepSizes[6]);
...
std::vector<uint32_t> decodeKeyPress_thirdset(uint16_t key_array){
  std::vector<uint32_t> stepSizeReturn;
  switch(key_array){
    case 0xE:
      stepSizeReturn.push_back(stepSizes[8]);
      noteMessage[2] = intToHex[8];
      break;
    case 0xD:
      stepSizeReturn.push_back(stepSizes[9]);
      noteMessage[2] = intToHex[9];
      break;
    case 0xB:
      stepSizeReturn.push_back(stepSizes[10]);
      noteMessage[2] = intToHex[10];
      break;
    case 0x7:
      stepSizeReturn.push_back(stepSizes[11]);
...
std::vector<uint32_t> checkKeyPress(uint16_t keyarray, uint8_t k3, uint8_t k4, uint8_t k5,uint8_t k6) {
  xSemaphoreTake(stepSizeMutex, portMAX_DELAY);
  std::vector<uint32_t> stepSizeReturn = currentStepSize; //default if none of the if conditions are met
  xSemaphoreGive(stepSizeMutex);
  
  std::vector<uint32_t> return_first, return_second, return_third;
  
  //get knob rotations
  //first get knobs AB values from the keyArray values passed into this function - local variables used so only one access to knob_x objects
  ...
  if (receiveMode == 0){
    if(keyarray == 0xFFF){
      if (!sustainMode){ //SUSTAIN MODE IMPLEMENTATION - only udates note with 0 if sustain mode is not on
        stepSizeReturn.clear();
        stepSizeReturn.push_back(0);
      }
      else{
        xSemaphoreTake(stepSizeMutex, portMAX_DELAY);
      ...
      noteMessage[0] = 'P';
      noteMessage[1] = o[0];
      if(((keyarray & 0xF00) >> 8) < 0xF){
        return_first = decodeKeyPress_firstset((keyarray & 0xF00) >> 8);
      }
      if(((keyarray & 0x0F0) >> 4) < 0xF){
        return_second = decodeKeyPress_secondset((keyarray & 0x0F0) >> 4);
      }
      if((keyarray & 0x00F) < 0xF){
        return_third = decodeKeyPress_thirdset(keyarray & 0x00F);
      }
      stepSizeReturn.clear();
      stepSizeReturn.reserve( return_first.size() + return_second.size() + return_third.size()); // preallocate memory
      stepSizeReturn.insert( stepSizeReturn.end(), return_first.begin(), return_first.end() );
      stepSizeReturn.insert( stepSizeReturn.end(), return_second.begin(), return_second.end() );
      stepSizeReturn.insert( stepSizeReturn.end(), return_third.begin(), return_third.end() );
    }
    xSemaphoreGive(modeMutex);
  }
...
void sampleISR() {
  ...
  xSemaphoreTake(joyMutex, portMAX_DELAY);
  int32_t locJoyValues[3];
  locJoyValues[0] = joyValues[0];
  locJoyValues[1] = joyValues[1];
  locJoyValues[2] = joyValues[2];
  xSemaphoreGive(joyMutex);
  
  xSemaphoreTake(stepSizeMutex, portMAX_DELAY);
  std::vector<uint32_t> loccurrentStepSize = currentStepSize;
  xSemaphoreGive(stepSizeMutex);
 
  xSemaphoreTake(modeMutex, portMAX_DELAY);
  bool locVibMode = vibratoMode;
  bool locTremMode = tremoloMode; 
  bool locSustainMode = sustainMode;
  xSemaphoreGive(modeMutex);

  
  int8_t locOctave = octave; //atomic operation so doesn't need semaphore
  ...
  Serial.println(loccurrentStepSize[0]);

  //adjusts frequency by counter gotten from vibrato lfo 
  if (locVibMode ==1){
    //stops clicking when no note is pressed and vibrato is on
      loccurrentStepSize[0]+= locVibCounter*100000; //100000 could be a parameter we change to get a wider/smaller range - shouldn't need to though as this can be done by protected variables in LFO class
  }
  ...
  if (locOctave>=4){
    loccurrentStepSize[0] = loccurrentStepSize[0] << (uint32_t) (locOctave-4);
  }
  else{
    loccurrentStepSize[0] = loccurrentStepSize[0] >> (uint32_t) (-1*(locOctave-4));
  }
  ...
  double freqAdjust = pow(2,1/12) * (locJoyValues[0]/distortResolution) * distortRange ; //frequency change between current note and next/prev 2 notes
   //gets rid of clicking when using joystick without pressing key
  loccurrentStepSize[0] += calcPhaseStep(freqAdjust);                        //setting the output
  
  phaseAcc += loccurrentStepSize[0];
  uint8_t outValue;
  ...
  if (locSustainMode&&(loccurrentStepSize[0]!=0)){
    ...
    while (Serial.available() > 0) {
      ...
      if (message_in != '\n') {
        ...
        if (inMsg[0] == 'R') {
          ...
          __atomic_store_n(&currentStepSize[0], stepSizes[std::stoi(key_in, 0, 16)], __ATOMIC_RELAXED);
        }
      }
    }
  }
...
void scanKeysTask(void * pvParameters) {
  ...
  while (1) {
    ...
    xSemaphoreTake(stepSizeMutex, portMAX_DELAY);
    std::vector<uint32_t> localCurrentStepSize = checkKeyPress(keysConcatenated, keyArray[3], keyArray[4], keyArray[5], keyArray[6]);
    xSemaphoreGive(stepSizeMutex);
    ...
    lfoVib.change_counterIncr(round(locJoyValues[1]/30));//updates vibrato speed based on joystick Y input 
    lfoVib.change_max(locknobsrot[0]/3);//updates vibrato range based on knob_1 rotational position
    
    
    if (locJoyValues[2] == 1){ //reset lfo by clicking joystick button if vibrato goes too far, and you don't want to spend all the time winding it back
      lfoVib.reset_counter();
      xSemaphoreTake(knobsMutex, portMAX_DELAY);
      knob_0.reset_knob_position(); //sephamore needed here
      xSemaphoreGive(knobsMutex);
    }
    xSemaphoreGive(lfoMutex);
    ...
    updateKeyboardRules(locknobsrot[0], locknobsrot[1], locknobsrot[2], locknobsrot[3], locJoyValues[2]);
  
    xQueueSend( msgOutQ, (char*) noteMessage, portMAX_DELAY);
    xSemaphoreTake(stepSizeMutex, portMAX_DELAY);
    currentStepSize.clear();
    currentStepSize.push_back(localCurrentStepSize[0]);
    xSemaphoreGive(stepSizeMutex);
...
void displayUpdateTask(void * pvParameters) {
  ...
  while (1) {
    ...
    //VIBRATO OPTIONS - SPEED AND RANGE
     ...
     xSemaphoreTake(lfoMutex, portMAX_DELAY);
     u8g2.print(lfoVib.get_incr(),DEC);
     xSemaphoreGive(lfoMutex);
    ...
    switch(waveformMode){
      ...
      case(4):
        ...
        break;
    }      // set coordinates to print knob values
    //u8g2.setFont(u8g2_font_ncenB08_tr);
    u8g2.drawStr(40,20,"Wave: ");
    u8g2.drawStr(73, 20, waveform.c_str());  //printing the wave type
    xSemaphoreGive(knobsMutex);

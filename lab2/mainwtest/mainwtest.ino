...
void setup() {
  ...
  */
  msgOutQ = xQueueCreate( 8, 3 );
  //for(int iter = 0; iter<8; iter++){
    //xQueueSend( msgOutQ, "ABCD" , portMAX_DELAY);
  //}
...
volatile bool sustainMode = true;
...
void msgOutTask(void *pvParameters) {
  ...
  Serial.println((micros()-startTime)/8);
  while (1) {
...
void displayUpdateTask(void * pvParameters) {
  ...
  vibratoMode = 2;
  tremoloMode = 2;
  Serial.println("why is this broken");
  int startTime = micros();
